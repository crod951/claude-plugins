...
2. Run the done-on-merge sweep for the resolved tracker; the mechanics are described in `../fathom-shared/trackers.md` and are tracker-agnostic, with each adapter file defining only its own closure action for the merged path.
   ...
   Otherwise, when any bundle reports `unknown`, change nothing about the issue and leave it for a later sweep: name the review ids that came back `unknown`, close nothing, restack nothing, and report no progress for the issue.
   ...
   This case sits below the `closed-unmerged` case, which closes nothing and restacks nothing whatever the undetermined bundles turn out to be, and above the three cases below, each of which judges the whole stack on all of its bundles at once and cannot do that with an undetermined bundle among them.
