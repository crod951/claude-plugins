...
2. Run the done-on-merge sweep for the resolved tracker; the mechanics are described in `../fathom-shared/trackers.md` and are tracker-agnostic, with each adapter file defining only its own closure action for the merged path.
   ...
   A bundle carrying only the `- Review: pending (bundle k/N)` marker described in `conventions.md` counts as a missing record here rather than as a review: the marker names no id, so there is nothing for `getReviewState` to be called on and nothing is known about that bundle's fate.
...
11. Once `claimNext` returns none remaining, finish the issue.
    ...
    - Write `- Review: pending (bundle k/N)` beneath that bundle's line in the plan document's `Bundles` section, and commit it on branch k, before calling `openReview` for this bundle.
      ...
      This marker is a write-ahead record of intent, and writing it ahead of the call is the whole point: after `openReview` has been entered, a run that dies leaves no other trace that the call was ever made.
    ...
    Decide that from that bundle's entry in the `Bundles` section and from nothing else, since the pending marker above is written and committed before `openReview` is called and is therefore the only record that separates a deliberate attempt from an accident.
    Exactly one of three cases holds for bundle k.
    ...
    When the entry carries no line at all beneath it, `openReview` was never attempted for this bundle, because the marker is committed before the call and no run reaches the call without it, so no review can exist for this bundle: write the marker as the routine above says, open the review, and skip the lookup below entirely.
    That is the ordinary case on a first run through a stack, which is why a first run never stops here.
    When the entry carries a `- Review: pending (bundle k/N)` marker and no id, the call was attempted and its outcome was never recorded, so a review may or may not exist: work through the lookup below, and hold wherever it cannot settle the question.
    Do not read the remote's branch list as evidence in any of these three cases, and never treat a branch the remote does not carry as proof that no review was opened from it.
    A forge retains a review after its head branch is deleted, and nothing in the lifecycle rules prevents a bundle branch being deleted or reused before its `- Review:` record is durable, so branch absence cannot tell a bundle that was never attempted from one whose review was opened and whose branch later went away.
    The pending marker carries that intent instead, and it is the only evidence this procedure accepts for it.
