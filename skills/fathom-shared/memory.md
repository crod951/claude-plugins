...
| `createTask(title, description, subIssueRef, deps)` | Create a task, tag it with the issue ref so all tasks for one issue can be listed directly, record `subIssueRef` on the task, and return its task id. When the issue was split into a stack, the first task of each bundle after the first also takes a dep on the last task of the previous bundle, so the backend itself enforces bundle order and `claimNext` cannot hand out a later bundle's task while an earlier bundle is still open. |
