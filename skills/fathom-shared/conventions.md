...
- **Bundles** - present only when this issue was split into a stack, and omitted entirely otherwise.
  ...
  Record a confirmation that none exists by replacing that bundle's marker with a `- Review: none confirmed (bundle k/N)` line, committed and pushed on the branch that run is on as the same kind of bookkeeping commit, so the next run opens that bundle's review instead of holding again.
  ...
  Each one lands on whichever branch the run is on when it writes the line, which in the ordinary routine is that bundle's own branch.
  A recovery is the exception, and deliberately so: a run recovering a bundle whose branch was deleted or reused records that bundle's outcome on the branch it is on instead, because that branch exists and is a write path while the bundle's own branch offers none.
  Either way the record is reachable, since the collection below fetches every branch named in this section rather than only the highest one and takes every `- Review:` line each of them carries.
