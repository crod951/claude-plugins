...
| --- | --- |
| `init(issueRef)` | Check whether `.issue-lifecycle/tasks/<issueRef>.md` exists. When it does not, create it with the header line, the `Issue`, `Branch`, and `PR` lines, an empty `## Plan` section, and an empty `## Tasks` section, then commit it. When it already exists, do nothing; never overwrite an existing file. |
...
| `close(taskId)` | Find the `## Tasks` line numbered `taskId`, rewrite its marker from `[>]` to `[x]`, and append `(done <date>)` to the end of the line. Commit this rewrite as part of, or immediately after, the commit that implements the task; do not leave it as a separate trailing commit on its own. |
